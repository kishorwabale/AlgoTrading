...
import pytz
from session_memory import SessionMemory   # v11 — Phase 1-3 session memory

# v11 — session-long scan history (stability %, flips, momentum, gating)
...
GOOGLE_CREDENTIALS_B64= os.getenv("GOOGLE_SHEETS_CREDENTIALS_B64", "")
...
# ── TEMP: Google Sheets disabled for signal-only testing ──────
...
def init_google_sheets():
    ...
    if not GOOGLE_SHEETS_ENABLED:
        print("⏸ Google Sheets disabled (testing mode) — signals still send to Telegram")
        return False

    if not GOOGLE_SHEET_ID or not (GOOGLE_CREDENTIALS or GOOGLE_CREDENTIALS_B64):
        print("⚠️ Google Sheets not configured — skipping auto-logging")
        return False

    try:
        # Parse credentials — prefer base64 (safe for multi-line JSON in .env files)
...
def place_super_order(name, strike, side, lot_size, entry_ltp, is_late=False, regime="NORMAL"):
    """
    ...
    → Target/SL sized dynamically by market regime (see REGIME_TARGET_SL)
    ...
    # Calculate target and SL — regime-aware, tightened further if late session
    target_pct, sl_pct, gain_pct_num, loss_pct_num = get_target_sl_pct(regime, is_late)
    session = "LATE" if is_late else regime
    ...
    try:
        ...
        if order_id:
            ...
            return order_id, (
                ...
                f"Target:       ₹{target_price} (+{gain_pct_num}%) → *+₹{gain_amt}*\n"
                f"Stop Loss:    ₹{sl_price} (-{loss_pct_num}%) → *-₹{loss_amt}*\n"
...
def handle_telegram_update(update):
    ...
    if text.startswith("/buy_"):
        ...
        if len(parts) >= 3:
            ...
            # Calculate target and SL for display — regime-aware
            regime_for_order = current_regime.get(name, "NORMAL")
            target_pct, sl_pct, gain_pct_num, loss_pct_num = get_target_sl_pct(regime_for_order, is_late)
            target = round(ltp * target_pct, 2)
            sl     = round(ltp * sl_pct, 2)
            tpct   = f"+{gain_pct_num}%"
            slpct  = f"-{loss_pct_num}%"
        ...
        if len(parts) >= 3:
            ...
            if order:
                ...
                order_id, msg_txt = place_super_order(
                    ...
                    regime=current_regime.get(name_o, "NORMAL")
                )
...
# ══════════════════════════════════════════════════════════════
# NIFTY PRE-OPEN AUCTION (9:00-9:15 F&O pre-open) — implied gap
# Merged in from the standalone premarket_update repo's
# futures_preopen_update.py so it shares this bot's already-live
# Dhan token instead of generating a second one, and avoids two
# separate processes hitting Dhan's API at the same 9:13 AM moment.
# ══════════════════════════════════════════════════════════════
NSE_HEADERS = {
    "User-Agent": (
...
_nifty_futures_sec_id_cache = {"id": None}
...
def _get_nifty_prev_close():
    """NIFTY 50's previous close, via NSE's allIndices endpoint (proven
    reliable from CI, unlike the more heavily bot-protected NSE
    endpoints) - used as the baseline for the pre-open implied gap."""
    s = _nse_session()
    r = s.get("https://www.nseindia.com/api/allIndices", timeout=5)
    r.raise_for_status()
    for row in r.json().get("data", []):
        if row.get("index") == "NIFTY 50":
            return float(row.get("previousClose", 0) or 0)
    return 0

def _get_nifty_futures_security_id():
    """
    Downloads Dhan's compact instrument master CSV and finds the current
    front-month NIFTY index futures contract's security ID (changes every
    monthly expiry, can't be hardcoded). Matches on the exact leading
    token of the trading symbol — a loose "contains NIFTY" match wrongly
    catches MIDCPNIFTY/NIFTYNXT50/NIFTYIT, which trade at very different
    levels and silently produce a nonsense implied gap.
    """
    if _nifty_futures_sec_id_cache["id"]:
        return _nifty_futures_sec_id_cache["id"]
    ...
    for row in rows:
        ...
        if instrument == "FUTIDX" and symbol_prefix == "NIFTY" and expiry >= today and sec_id:
    ...
    candidates.sort()
    _nifty_futures_sec_id_cache["id"] = candidates[0][1]
    return candidates[0][1]

def get_futures_preopen_gap():
    """
    Nifty near-month futures price from the 9:00-9:15 pre-open auction,
    compared against NIFTY 50's previous close for an implied gap - same
    idea as Gift Nifty, but from the domestic pre-open auction. Returns a
    dict with ok=False and an error string on any failure - caller should
    treat this as best-effort and never let it block the main signal loop.
    """
    try:
        sec_id = _get_nifty_futures_security_id()
        data = _dhan_quote({"NSE_FNO": [int(sec_id)]})
        d = data.get("data", {}).get("NSE_FNO", {}).get(str(sec_id), {})
        if not d:
        ...
        # Sanity check — if these are more than ~10% apart, the wrong
        # contract was almost certainly matched (this is how the
        # MIDCPNIFTY mismatch first surfaced), not a real market move.
        if abs(fut_price - prev_close) > 0.10 * prev_close:
            return {
                "ok": False,
                "error": (
                    f"futures_price {fut_price} implausibly far from prev_close "
                    f"{prev_close} (security_id={sec_id}) — likely wrong contract"
                ),
            }

        gap = round(fut_price - prev_close, 2)
        return {"futures_price": fut_price, "prev_close": prev_close, "gap_points": gap, "ok": True}
    except Exception as e:
        return {"ok": False, "error": str(e)}

def send_futures_preopen_alert():
    """Formats and sends the pre-open auction confirmation - called once
    during the pre-market phase, right after the existing global-markets
    brief, since by the time this bot's startup sequence reaches this
    point the 9:08-9:12 auction has already concluded."""
    fp = get_futures_preopen_gap()
    if not fp.get("ok"):
        print(f"⚠️ Futures pre-open fetch failed: {fp.get('error')}")
        return
...
def parse_oc(raw):
    ...
    if not chains:
        print(f"  ⚠️ OC API returned no chain data. Raw keys: {list(raw.keys())}, data type: {type(d)}, spot: {spot}")
        return spot, []
    parsed = []
    if isinstance(chains, dict):
        # Dhan v2 actual format: {"23900.000000": {"ce": {...}, "pe": {...}}, ...}
        for strike_key, row in chains.items():
            ...
            try:
                ...
                continue
            ce = row.get("ce", row.get("CE", {})) or {}
            pe = row.get("pe", row.get("PE", {})) or {}
            parsed.append({
                "strike": strike,
                "ceLTP":  ce.get("last_price", ce.get("lastTradedPrice", ce.get("ltp", 0))),
                "ceOI":   ce.get("oi",         ce.get("openInterest",    0)),
                "ceIV":   ce.get("implied_volatility", ce.get("impliedVolatility", ce.get("iv", 0))),
                "peLTP":  pe.get("last_price", pe.get("lastTradedPrice", pe.get("ltp", 0))),
                "peOI":   pe.get("oi",         pe.get("openInterest",    0)),
                "peIV":   pe.get("implied_volatility", pe.get("impliedVolatility", pe.get("iv", 0))),
            })
    else:
        # Legacy/fallback format: list of row dicts
        for row in chains:
            if not isinstance(row, dict):
                continue
...
# ══════════════════════════════════════════════════════════════
# TWO-TIER SIGNAL CONFLUENCE — EARLY vs CONFIRMED
#
# The old 4-band system granted "Full size" the instant a single
# 5-min cycle scored >= 75. One noisy print could look identical to
# a real move. This layer sits on top and only changes what happens
# at that top band — WATCH/CAUTION/SKIP bands below it are untouched.
#
# EARLY   — first strong cycle at this side/score. Real signal, but
#           unconfirmed. Suggested: partial size, so you're in if
#           it's real without full risk if it's noise.
# CONFIRMED — same side has now persisted >=2 consecutive cycles at
#           CAUTION+ (>=65), the OI buildup pattern agrees with the
#           direction, AND spot has actually moved that way since the
#           earliest cycle in the window. All three, not just one.
# ══════════════════════════════════════════════════════════════
BULLISH_OI_PATTERNS = {"LONG BUILDUP", "SHORT COVER"}
BEARISH_OI_PATTERNS = {"SHORT BUILDUP", "LONG UNWIND"}

signal_history = {}  # {name: deque[(side, score, spot)]} — last 3 cycles (~15 min)

# ══════════════════════════════════════════════════════════════
# PRICE MOMENTUM LAYER
#
# The core engine reads positioning (OI/PCR/Max Pain) — those are
# *reaction* indicators that lag price by design, which is why a
# clean sequential fall (e.g. 2-Jul: 24,155 → 24,077 over six red
# 5-min candles) produced no PE signal: writers hadn't repositioned
# yet, and the EXPIRY regime further muted directional inputs.
#
# This layer adds raw spot velocity as its own score adjustment:
# tracked over the last ~3 cycles (~15 min), applied DIRECTLY to
# ce_s/pe_s AFTER regime weighting, deliberately bypassing regime
# weights — a 0.3% move in 15 minutes is information in any regime.
#
# Known cost: will also reward some head-fakes that OI correctly
# ignores. That's the tradeoff for catching real reversals earlier;
# the confluence tier system above still gates full-size on OI+price
# agreement, so this mostly surfaces the move sooner rather than
# instantly granting full size.
# ══════════════════════════════════════════════════════════════
spot_momentum_history = {}  # {name: deque[spot]} — one entry per cycle

def compute_momentum_adj(name, spot):
    """
    Returns (ce_adj, pe_adj, note). Positive spot velocity boosts CE
    and dampens PE; negative boosts PE and dampens CE. Needs 3+ cycles
    of history (~10-15 min) before it activates.
    """
    hist = spot_momentum_history.setdefault(name, deque(maxlen=4))
    hist.append(spot)
    if len(hist) < 3:
        return 0, 0, None

    oldest = hist[0]
    chg_pct = (spot - oldest) / oldest * 100 if oldest else 0

    if chg_pct >= 0.30:
        return 15, -8, f"🚀 MOMENTUM: spot +{chg_pct:.2f}% in ~15m — strong up-move"
    if chg_pct >= 0.18:
        return 10, -5, f"📈 MOMENTUM: spot +{chg_pct:.2f}% in ~15m — up-move building"
    if chg_pct <= -0.30:
        return -8, 15, f"🔻 MOMENTUM: spot {chg_pct:.2f}% in ~15m — strong down-move"
    if chg_pct <= -0.18:
        return -5, 10, f"📉 MOMENTUM: spot {chg_pct:.2f}% in ~15m — down-move building"
    return 0, 0, None

def classify_confluence_tier(name, sig, oi_pattern, spot):
    """
    Call only when this cycle's best score >= 75 (the existing TRADE
    band). Returns (tier, note) where tier is "EARLY" or "CONFIRMED".
    """
    side  = "CE" if sig["ce_score"] >= sig["pe_score"] else "PE"
    score = max(sig["ce_score"], sig["pe_score"])

    hist = signal_history.setdefault(name, deque(maxlen=3))
    hist.append((side, score, spot))

    if len(hist) < 2:
        return "EARLY", "First strong print — no prior cycle to confirm against yet"

    persistent = all(h_side == side and h_score >= 65 for h_side, h_score, _ in hist)
    if not persistent:
        return "EARLY", "Side/score not consistent across recent cycles"

    oi_agrees = (
        (side == "CE" and oi_pattern in BULLISH_OI_PATTERNS) or
        (side == "PE" and oi_pattern in BEARISH_OI_PATTERNS)
    )
    if not oi_agrees:
        return "EARLY", f"OI pattern ({oi_pattern}) doesn't confirm {side} yet"

    earliest_spot = hist[0][2]
    price_moved = (spot > earliest_spot) if side == "CE" else (spot < earliest_spot)
    if not price_moved:
        return "EARLY", "Price hasn't confirmed direction yet"

    return "CONFIRMED", "Multi-cycle + OI + price all agree"
...
# ══════════════════════════════════════════════════════════════
# DYNAMIC TARGET/SL BY REGIME
# Flat +30%/-25% never triggers on typical range-bound days
# (Nifty/Sensex/BankNifty often move <0.6% intraday). Size the
# target/SL band to how much movement the regime actually implies.
#   (target_gain_pct, sl_loss_pct)
# ══════════════════════════════════════════════════════════════
REGIME_TARGET_SL = {
    "EXPIRY":         (18, 15),  # pins toward Max Pain — small realistic moves
    "VOLATILE_TREND": (35, 25),  # biggest expected moves — keep wide
    "TRENDING":       (25, 20),  # moderate-strong directional moves
    "RANGE_NEUTRAL":  (15, 12),  # tight range, Max Pain dominates
    "RANGE_BIASED":   (18, 15),  # rangebound with mild bias
    "HIGH_VIX":       (30, 22),  # bigger swings but two-sided/choppy
    "NORMAL":         (22, 18),  # fallback baseline
}

def get_target_sl_pct(regime, is_late=False):
    """
    Returns (target_pct, sl_pct) as multipliers on entry price, e.g.
    target_pct=1.25 means +25%, sl_pct=0.80 means -20%.
    Late session further tightens the band (less time for the move to play out).
    """
    gain_pct, loss_pct = REGIME_TARGET_SL.get(regime, REGIME_TARGET_SL["NORMAL"])
    if is_late:
        gain_pct = max(10, round(gain_pct * 0.65))
        loss_pct = max(8,  round(loss_pct * 0.60))
    return round(1 + gain_pct/100, 4), round(1 - loss_pct/100, 4), gain_pct, loss_pct
...
def compute_signals(name, spot, data, ce_added, pe_added, bonus,
    ...
    # Applied after regime weighting on purpose — raw spot velocity
    # counts in every regime, including EXPIRY where directional
    # inputs are otherwise muted. See momentum layer comment above.
    mom_ce, mom_pe, mom_note = compute_momentum_adj(name, spot)
    if mom_ce or mom_pe:
        ce_s = min(100, max(0, ce_s + mom_ce))
        pe_s = min(100, max(0, pe_s + mom_pe))
...
def format_message(results, buildups, top_chg, ping, resp,
    ...
    # Window label
    window_labels = {
        "OPENING":         "⚠️ OPENING WINDOW (9:15–9:30) — Let market settle, DO NOT trade yet",
        "BEST_WINDOW":     "✅ BEST ENTRY WINDOW (9:30–11:00 AM)",
        "SLOW_ZONE":       "⚠️ SLOW ZONE — Score ≥80 only",
        "ACTIVE":          "✅ ACTIVE ZONE (1:00–2:30 PM)",
        "LATE_SESSION":    "⚡ LATE SESSION (2:30–3:30 PM) — Score ≥85 · Strict rules",
        "EXPIRY_CAUTION":  "⚠️ EXPIRY CAUTION (2–3 PM) — Theta heavy · Trade small, exit fast",
        "EXPIRY_EXIT":     "🔴 EXPIRY EXIT — Exit all now",
        "EXIT_ALL":        "🔴 EXIT ALL — No new entries",
    }
    w_label = window_labels.get(window, "")

    lines.append(f"📡 *OC RADAR v11* · {now_ist}")
    ...
    for n, s in results.items():
        ...
        if top >= 75:
            ...
            band = "🔴 SKIP"
        # v11 — Phase 2 momentum arrow (↗ building / ↘ fading / → flat)
        arrow = session_mem.momentum(n)
        lines.append(f"{n:<12} {side_lbl} {top:>3}{arrow} {band}")
        if top > best_score:
            best_score = top
            best_name  = n
            best_side  = side_lbl
    ...
    # v11 — Phase 1: session context (flip count + direction stability)
    ctx_lines = [session_mem.header_line(n) for n, s in results.items() if s]
    if ctx_lines:
        lines.append("📈 *SESSION CONTEXT*")
        lines.extend(ctx_lines)

    # Composite  (v11 fix: `or {}` — a failed index stores None and
    # results.get(name, {}) still returns None, crashing .get())
    comp_ce = round(
        (results.get("BANKNIFTY") or {}).get("ce_score",0)*0.40 +
        (results.get("NIFTY")     or {}).get("ce_score",0)*0.35 +
        (results.get("SENSEX")    or {}).get("ce_score",0)*0.25
    )
    comp_pe = round(
        (results.get("BANKNIFTY") or {}).get("pe_score",0)*0.40 +
        (results.get("NIFTY")     or {}).get("pe_score",0)*0.35 +
        (results.get("SENSEX")    or {}).get("pe_score",0)*0.25
    )
    ...
    for name, sig in results.items():
        ...
        if score >= 75:
            tier = sig.get("confluence_tier")
            if tier == "CONFIRMED":
                band_emoji = "🌟"; band_label = "CONFIRMED"
            elif tier == "EARLY":
                band_emoji = "⚡"; band_label = "EARLY"
            else:
        ...
        # v11 — Phase 3 gate: suppress conflicting setups in chop.
        # TRADE band (>=75) is never gated inside gate() itself.
        gate_dir = "CE" if sig["ce_score"] >= sig["pe_score"] else "PE"
        gate_action, gate_reason = session_mem.gate(name, gate_dir, score)
        if gate_action == "CHOP":
            chop_exp_raw = expiries.get(name, "")
            chop_exp     = fmt_date(chop_exp_raw) if chop_exp_raw else "—"
            lines.append(f"\n📋 *{name}* ₹{sig['spot']:,.0f} · {chop_exp}")
            lines.append(f"⚠️ *CHOP* — {gate_reason}")
            lines.append(f"MP ₹{sig['mp']:,} · *NO EDGE — stand aside*")
            lines.append("━━━━━━━━━━━━━━━━━━━━━━")
            continue
        ...
        # Targets — regime-aware (falls back to NORMAL band if regime unknown)
        regime_here = current_regime.get(name, "NORMAL")
        target_pct, sl_pct, tgt_pct_num, sl_pct_num = get_target_sl_pct(regime_here, is_late)
        tgt = round(entry * target_pct); sl = round(entry * sl_pct)
        tgt_lbl = f"+{tgt_pct_num}%"; sl_lbl = f"-{sl_pct_num}%"
        session_note = "⚡ Late Session · Exit by 3:05 PM" if is_late else ""
        ...
        if sig.get("confluence_note"):
            lines.append(f"  {'⏳' if sig.get('confluence_tier')=='EARLY' else '✅'} {sig['confluence_note']}")
        ...
        # Price momentum — spot velocity over ~15 min
        if sig.get("momentum_note"):
            lines.append(sig["momentum_note"])
        ...
        lines.append(f"Band:         {regime_here} regime")
        if session_note:
            lines.append(session_note)

        # Trade band indicator — no /buy command in message (manual entry only)
        # v11 fix: respect confluence tier — EARLY must not say "Full size"
        if score >= 75:
            if sig.get("confluence_tier") == "EARLY":
                lines.append(f"\n⚡ *EARLY* — Partial size only (confirmation pending)")
            else:
                lines.append(f"\n✅ *TRADE* — Full size")
        elif score >= 65:
        ...
        # v11 — fading-score flag from session memory
        if gate_action == "FADE":
            lines.append("⚠️ _Score fading over last 3 scans_")
    ...
    lines.append("_OC Radar v11 · Educational only_")
    return "\n".join(lines)
...
def send_market_open_alert(expiries, global_data):
    ...
    msg   = (
        ...
        "_OC Radar v11 · Educational only_"
    )
    send_telegram(msg)
...
def get_time_window(is_expiry_day=False):
    ...
    Extended to 3:30 PM for non-expiry days
    Expiry day hard stop at 2:00 PM
    OPENING window (9:15-9:30) now sends signals too, but tagged as
    "let market settle" via the window_labels banner in format_message() -
    informational, not meant for new entries until BEST_WINDOW at 9:30.
    """
    now = datetime.now(IST)
    t   = now.time()
    ...
    if is_expiry_day:
        ...
        elif t < dtime(9, 30):  return "OPENING",        True,  MIN_SCORE
        ...
        # v11 change — signals now continue till 3:00 PM on expiry day.
        # EXPIRY_CAUTION (2:00-3:00) still sends, banner warns user.
        elif t < dtime(15, 0):  return "EXPIRY_CAUTION", True,  MIN_SCORE
        ...
        elif t < dtime(15, 30): return "LATE_SESSION",    True,  MIN_SCORE
...
def check_partial_booking(results):
    ...
    for name, sig in results.items():
        ...
        # Reset tracking if this isn't the same contract we were tracking —
        # either side flipped (CE<->PE) OR the ATM strike itself shifted
        # (routine as spot moves through the day). Missing the strike check
        # here meant the old entry price of one strike got compared against
        # the live premium of a completely different strike, producing a
        # bogus P&L and false SL/target alerts.
        if trade["side"] != side or trade["strike"] != strike:
            active_trades[name] = {
                "side":   side,
                "entry":  current_ltp,
                "strike": strike,
    ...
    for a in alerts:
        if a["type"] == "PARTIAL":
            msg = (
                ...
                f"━━━━━━━━━━━━━━━━━━━━━━\n"
                f"_OC Radar v11 · Educational only_"
            )
        elif a["type"] == "TARGET":
            msg = (
...
def main():
    print("🚀 OC Radar Bot v10 started!")
    ...
    if t_now < dtime(9, 15):  # Wait till market opens
        ...
        lines.append("━━━━━━━━━━━━━━━━━━━━━━")
        lines.append("📡 *First signal (informational) at 9:15 AM*")
        lines.append("⚠️ 9:15–9:30 AM — Let market settle, DO NOT trade")
        lines.append("✅ *Trade-ready signals from 9:30 AM*")
        lines.append("\n_OC Radar v11 · Educational only_")

        send_telegram("\n".join(lines))
        print("✅ Pre-market brief sent!")

        # Nifty pre-open auction (9:00-9:15) confirmation — by the time
        # this bot's own startup sequence reaches this point, the auction's
        # order-matching phase (9:08-9:12) has already concluded, so the
        # discovered price is already stable and available.
        send_futures_preopen_alert()
    ...
    while True:
        ...
        try:
            ...
            # Exit alert — 3:00 PM on expiry (v11: was 2:30, signals now
            # run till 3:00), 3:20 PM on normal day
            exit_time = dtime(15, 0) if is_expiry_day else dtime(15, 20)
            exit_time_end = dtime(15, 4) if is_expiry_day else dtime(15, 24)
            ...
            if t < dtime(0, 1):
                ...
                spot_momentum_history.clear()
                session_mem.history.clear()   # v11 — fresh session memory daily
            ...
            for name, cfg in INDICES_BASE.items():
                ...
                try:
                    ...
                    if not data:
                        print(f"  ❌ {name}: no option data (expiry sent: '{expiries.get(name,'')}', spot: {spot})")
                    ...
                    # Two-tier confluence check — only kicks in at the
                    # existing TRADE band (score >= 75). WATCH/CAUTION/
                    # SKIP bands below are untouched.
                    if sig and max(sig["ce_score"], sig["pe_score"]) >= 75:
                        tier, tier_note = classify_confluence_tier(name, sig, pat, spot)
                        sig["confluence_tier"] = tier
                        sig["confluence_note"] = tier_note
                        if tier == "EARLY":
                            sig["size"] = "⚡ EARLY — Partial size (confirmation pending)"
                        else:
                            sig["size"] = "🌟 CONFIRMED — Full size"

                    results[name] = sig

                    # v11 — record scan into session memory (Phase 1-3).
                    # Must run for every index every cycle, even WATCH/SKIP,
                    # and must run BEFORE format_message() calls gate().
                    if sig:
                        rec_dir = "CE" if sig["ce_score"] >= sig["pe_score"] else "PE"
                        session_mem.record(
                            name, rec_dir,
                            max(sig["ce_score"], sig["pe_score"]),
                            sig["spot"]
            ...
            # DEBUG: show live scores every cycle, even below threshold
            score_bits = []
            for name, sig in results.items():
                if sig:
                    score_bits.append(f"{name} CE:{sig['ce_score']} PE:{sig['pe_score']}")
                else:
                    score_bits.append(f"{name}:ERR")
            print("  📈 Scores — " + " | ".join(score_bits))
